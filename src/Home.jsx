...
export default function Home({ onSelect }) {
    ...
    const fetchControl = async () => {
      try {
        ...
        const visiblePlayers = rows
          ...
          // 🔤 Ordena alfabèticament pel playerName
          .sort((a, b) =>
            a.playerName.localeCompare(b.playerName, "ca", { sensitivity: "base" })
          );
  ...
  if (loading)
    return <div className="p-6 text-black dark:text-white">Loading players…</div>;
  ...
  if (error)
    return (
      <div className="p-6 text-black dark:text-white">
        ...
        <p>
          Revisa que el full “{CONTROL_SHEET_NAME}” sigui públic i tingui dades a
          A:C.
        </p>
      </div>
    );
  ...
  if (players.length === 0)
    return (
      <div className="p-6 text-black dark:text-white">
        No active players (Visible = YES) found in Control.
      </div>
    );
